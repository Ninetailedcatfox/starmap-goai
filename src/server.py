...
## 输出格式（严格JSON，无其他文本）
{
  ...
  "careers": [
    {"name":"职业名","match":82,"desc":"40字描述","salary":"薪资区间","growth":"行业趋势","schools":["校1","校2","校3"]}
  ],
  "timeline_stages": [
...
class APIHandler(SimpleHTTPRequestHandler):
    def do_POST(self):
        ...
        user_msg = f"选科:{', '.join(subjects) if subjects else '无'}\n兴趣:{interests or '无'}\n自述:{self_desc or '无'}\n输出JSON。"

        try:
            resp = client.chat.completions.create(model=MODEL, messages=[{"role":"system","content":SYSTEM_PROMPT}, {"role":"user","content":user_msg}], temperature=0.3, timeout=60)
...
if __name__ == "__main__":
    ...
    print("StarMap -> http://localhost:8800")
    ThreadingHTTPServer(("0.0.0.0", 8800), APIHandler).serve_forever()
